resource "aws_security_group" "wazuh-server-sg" {
  name        = "rd-wazuh-server-sec-gp"
  ...
  tags = {
    Name = "wazuh-server-sgp"
  }
}

resource "tls_private_key" "wazuh-server-key" {
  algorithm = "RSA"
  rsa_bits  = 2048
}

resource "aws_key_pair" "wazuh-server-keypair" {
  key_name   = var.ec2-instance.server_key_name
  public_key = tls_private_key.wazuh-server-key.public_key_openssh
}
...
resource "aws_instance" "wazuh_server" {
  ...
  instance_type               = var.ec2-instance.server_instance_type
  key_name                    = aws_key_pair.wazuh-server-keypair.key_name
  subnet_id                   = var.public_subnet
  vpc_security_group_ids      = [aws_security_group.wazuh-server-sg.id]
